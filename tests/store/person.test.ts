...
test("person_create", async () => {
  ...
  assert.equal(
    ...
    personData.additionalName,
    "Additional names are not equal",
  );
  assert.deepEqual(
    person.dateOfBirth,
    personData.dateOfBirth,
    "Date of births are not equal",
  );
});
...
test("person_dateOfDeath", async () => {
  ...
  assert.deepEqual(
    person.dateOfDeath,
    personData.dateOfDeath,
    "Date of deaths are not equal",
  );

  const changedDateOfDeath = {
    custom: true,
    date: faker.date.past().toISOString(),
  };
...
test("person_dateOfDeath_earlier_than_dateOfBirth", async () => {
  const personData = {
    ...
    dateOfDeath: {
      custom: false,
      date: faker.date.future({ years: 100 }).toISOString(),
    },
  };
  ...
  assert.throws(
    async () => (await personGenerator.next()).value,
    Error,
    undefined,
...
test("person_dateOfDeath_later_than_today", async () => {
  ...
  assert.throws(
    ...
    Error,
    undefined,
    "Date of death is later than today",
  );
});
...
test("person_dateOfBirth_later_than_today", async () => {
  ...
  assert.throws(
    ...
    Error,
    undefined,
    "Date of birth is later than today",
  );
});
...
test("person_marriageHash", async () => {
  ...
  const marriagePersonHash = (await marriagePersonGenerator.next())
    .value as string;
...
test("person_getFullNameAbbr", async () => {
  ...
  const expectedFullNameAbbr = `${personData.firstName.charAt(0)}. ${
    personData.additionalName != null
      ? `${personData.additionalName.charAt(0)}. `
      : ""
  }${personData.lastName}${
    personData.familyName != null ? ` - ${personData.familyName}` : ""
  }`;

  assert.equal(
    person.getFullNameAbbr(),
    expectedFullNameAbbr,
    "Full name abbreviations are not equal",
  );
});
...
test("person_edit", async () => {
  ...
  assert.equal(
    person.firstName,
    edits.firstName,
    "Edited first name is not equal",
  );
  assert.equal(
    person.lastName,
    edits.lastName,
    "Edited last name is not equal",
  );
  assert.equal(
    person.additionalName,
    edits.additionalName,
    "Edited additional name is not equal",
  );
  assert.deepEqual(
    person.dateOfBirth,
    edits.dateOfBirth,
    "Edited date of birth is not equal",
  );
});
...
test("person_remove_child", async () => {
  ...
  assert.isTrue(
    familyStore.has({ person: child, hash: childHash }),
    "Child was not added",
  );
