...
test("notifications_sendNotification", async () => {
    ...
    assert.equal(notifications?.get(0)?.level, NotificationLevel.Info, "Notification level is not correct");
    assert.equal(notifications?.get(0)?.message, 'Test message', "Notification message is not correct");
});
...
test("notifications_sendError", async () => {
    ...
    assert.equal(notifications.size, 1, "Notification count is not correct");
    assert.equal(notifications?.get(0)?.level, NotificationLevel.Error, "Notification level is not correct");
    assert.equal(notifications?.get(0)?.message, 'Test error message', "Notification message is not correct");
});
...
test("notifications_changeVisibility", async () => {
    ...
    assert.equal(notifications?.get(0)?.visible, false, "Notification visibility is not correct");
});

test("notifications_changeVisibility_invalidId", async () => {
    const notificationsStore = createNotifications();

    assert.throws(() => notificationsStore.changeVisibility(1, false), Error, "Notification error");
});
