...
test("families_add_same_person", async () => {
  ...
  assert.throws(
    () => familiesStore.add(person1, person1),
    undefined,
...
test("families_add_remove_child", async () => {
  ...
  assert.isTrue(
    ...
    "Families children store size should be 0",
  );
  ...
  assert.isTrue(
    ...
    "Families children store size should be 1",
  );
  ...
  assert.isTrue(
    childrenArray?.length === 0,
    "Child was not deleted as the size should be 0.",
  );
